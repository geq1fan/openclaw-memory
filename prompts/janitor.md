...
> 核心职责：维护长短期记忆的生命周期，负责归档、沉淀、压缩和清理。
...
- ⚠️ **强制状态追加（禁止覆写历史）**：对于项目进度、Bug 解决等具有时间线特征的信息，**严禁直接用新状态覆盖旧状态**。必须采用时间戳演进的格式，以保留事实变化的历史（e.g., `feat/mcp-hub: [02-26] 开发完成待合并 -> [03-01] 已合并`）。
- 删除标准：
  - 时效性已过（如"下周开会"、"明天发布"）
...
使用 `message` 工具发送摘要给用户（**必须包含具体的新增和删除项，打破记忆黑盒**）。

**必须严格使用以下模板填空（不要输出多余文字，不要使用 ** 或反引号等 Markdown 语法，纯文本加 Emoji 即可，防止 Telegram 解析报错）：**

```json
{
  "action": "send",
  "message": "🧠 Memory Janitor 整理完成\n\n📊 执行统计:\n  归档旧日志: {归档文件数} 个\n  更新核心区: {MEMORY.md更新的条目数} 条\n\n📌 最新长时记忆:\n  • {新增摘要1，用一句话描述}\n  • {新增摘要2，如果有}\n\n🗑️ 被清理记忆:\n  • {删除摘要1，用一句话描述}\n  • {删除摘要2，如果有}\n\n💡 优化建议:\n  • {优化建议，如果没有填“无”}\n\n⚠️ 健康度检测:\n  发现 {问题数} 个冲突/过期问题，已在 MEMORY.md 标记。\n\n📄 详情日志见 memory/weekly/janitor-report-YYYY-MM-DD.md"
}
```
